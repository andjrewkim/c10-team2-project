#!/usr/bin/env python3
"""Realtime IMU segment classifier GUI using a sliding window + SVM.

Collect 1-second IMU segments, train a linear SVM, and classify live data.
Ported from ``lab04-imu-lab/IMU_lab_students/tools/IMU_Classifier/python_imu_segment_demo_student.py``.
"""
...
class ImuSegmentFeatureExtractor:
    ...
    def capture_instance(
        ...
        stds = resampled.std(axis=0)
        mins = resampled.min(axis=0)
        maxs = resampled.max(axis=0)
        ranges = maxs - mins
        rms = self.np.sqrt((resampled ** 2).mean(axis=0))
        diffs = self.np.abs(self.np.diff(resampled, axis=0)).mean(axis=0)

        summary = self.np.concatenate([stds, mins, maxs, ranges, rms, diffs]).astype("float32")
...
class MLClassifier:
    ...
    def train(self, instances_by_label: dict[str, list[DataInstance]]) -> None:
        ...
        classifier.fit(self.deps.vstack(x_values), self.deps.asarray(y_values))
...
class ImuSegmentClassifierApp:
    ...
    def __init__(self, deps: RuntimeDeps, args: argparse.Namespace) -> None:
        ...
        self.root = tk.Tk()
        self.root.title(f"IMU Segment Classifier ({self.args.segment_seconds:g}s)")
    ...
    def _feature_config(self) -> dict[str, Any]:
        return {
            ...
            "feature_version": "student_with_stats_v1",
        }
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Realtime IMU segment classifier GUI.")
