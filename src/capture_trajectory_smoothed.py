#!/usr/bin/env python3
"""Capture a short IMU window on keypress and plot raw and smoothed trajectories.

Ported from ``lab04-imu-lab/IMU_lab_students/tools/capture_trajectory_smoothed_TODO.py``.
The ``MovingAverageFilter`` implementation is imported from
``src.sensors.lab_integration.imu``.
"""
...
def compute_trajectory_from_samples(capture_samples, baseline_samples, args):
...
def compute_raw_and_smoothed_trajectories(capture_samples, baseline_samples, args):
    raw_result = compute_trajectory_from_samples(capture_samples, baseline_samples, args)
    ...
    smoothed_result = compute_trajectory_from_samples(smoothed_capture, smoothed_baseline, args)
